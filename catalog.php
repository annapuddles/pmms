...
if (isset($_GET["query"])) {
	...
	if (isset($_GET["series"])) {
		...
		if (isset($_GET["category"])) {
			if (isset($_GET["genre"])) {
				$stmt = $conn->prepare("SELECT id, url, title, cover FROM catalog_with_genre WHERE series IS NULL AND category = ? AND genre = ? AND hidden = FALSE ORDER BY sort_title");
			...
			if (isset($_GET["genre"])) {
				$stmt = $conn->prepare("SELECT id, url, title, cover FROM catalog_with_genre WHERE series IS NULL AND genre = ? AND hidden = FALSE ORDER BY sort_title");
